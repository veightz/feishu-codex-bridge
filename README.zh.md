...
把飞书 / Lark 消息和本地 coding-agent CLI 打通的轻量 bot，用一条命令起服务，扫码绑应用，在飞书里和 Claude Code 或 Codex 对话、让它读图 / 改代码。
...
- **会话延续**：每个 chat 独立保存原生 agent session，对话能接着上次说
- **Claude Code + Codex**：实例有默认 agent，也可以在每个 chat / 话题里用 `/agent claude`、`/agent codex`、`/agent default` 动态切换；切换不会清空 bridge 保存的短上下文
- **抢占 + 批处理**：中途发新消息会打断旧任务；快速连发几条会合并成一次请求
- **多工作空间**：`/ws` 切换不同项目，session 自己重置
- **图片 / 文件**：直接发给 bot，当前 agent 会读本地下载的文件路径
- **卡片按钮**：`/help` `/ws list` `/status` 返回交互卡片，点按钮直接操作

## 前置条件

- Node.js **≥ 20**
- 至少安装并登录一个本地 agent CLI：
  - `claude`：https://docs.anthropic.com/en/docs/claude-code/quickstart
  - `codex`：如果还没登录，先运行 `codex login`
...
这个 fork 额外提供了一个可执行命令名：`feishu-codex-bridge`，用于本地并行测试。想不影响已经安装的原版 `lark-channel-bridge` 时，建议用这个命令配合命名实例：
...
如果你要保留全局原版命令，不要把这个 fork 以同一个 package name 全局安装覆盖上去。npm 的全局安装按 package/bin 建 shim，安装同名 `lark-channel-bridge` fork 可能替换全局 `lark-channel-bridge` 命令。无侵入本地测试时，直接跑上面的 checkout wrapper，或在自己的 shell 里给这个 repo 的 `bin/feishu-codex-bridge.mjs` 配 alias。
...
别名 `feishu-codex-bridge` 支持同样的子命令和参数。
...
后台 daemon 用 `./bin/feishu-codex-bridge.mjs start --instance codex-dev`。它会写入不同的 OS 服务名，不会和原版默认实例抢服务控制项：macOS 是 `ai.lark-channel-bridge.bot.codex-dev`，Linux 是 `lark-channel-bridge.bot.codex-dev.service`，Windows 是 `LarkChannelBridge.Bot.codex-dev`。这个 bridge 没有本地 HTTP server 端口，所以不存在端口占用；主要共享点是飞书 / Lark app 的 WebSocket 连接和本机进程注册表。如果两个实例绑定同一个 app id，bridge 会提示冲突，因为开放平台可能把事件随机路由到任意一个连接。
...
| `/resume [N]` | 查看 / 恢复当前 agent 和 cwd 下最近的原生会话 |
| `/agent [claude\|codex\|default]` | 查看或切换当前 chat / 话题的 agent，不清空 bridge 短上下文 |
...
| `/doctor [描述]` | 把最近运行日志和你的描述喂给默认 agent，自助诊断卡住 / 异常的原因 |
| `/help` | 帮助卡片 |
| 其它 `/xxx` | 原样交给当前 agent |
...
| `~/.lark-channel/sessions.json` | 每个 chat / 话题的原生 agent session/thread id + cwd（+ 可选的 `/timeout` 覆盖和 bridge 短上下文） |
...
命名实例的本地文件在 `~/.lark-channel/instances/<name>/`，可以把上表里的 `~/.lark-channel/` 替换成这个前缀理解。
...
**agent 挂住不回复**：通常是当前选择的 CLI 没登录、没在 `PATH` 里，或者 session 指向了不存在的 cwd。发 `/status` 看当前状态；`/agent` 看可用性；`/new` 重开会话往往就好。
